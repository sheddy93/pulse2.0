      ...
      <div className="p-6 max-w-3xl mx-auto space-y-5">
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
            <h2 className="font-semibold text-slate-800">Nuova richiesta</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                ...
                <textarea value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))}
                  rows={3} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" />
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          ...
          {leaves.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="text-sm text-slate-400 mt-1">Crea la tua prima richiesta di ferie o permesso</p>
            </div>
          ) : (
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-slate-500 mt-0.5">
                          Dal {format(new Date(l.start_date), "d MMM", { locale: it })} al {format(new Date(l.end_date), "d MMM yyyy", { locale: it })}
                          {l.days_count && <span className="ml-2 text-slate-400">({l.days_count} giorni)</span>}
                        </p>
                        {l.note && <p className="text-xs text-slate-400 mt-1">{l.note}</p>}
                        {l.admin_note && <p className="text-xs text-blue-600 mt-1">Nota HR: {l.admin_note}</p>}
                      </div>
